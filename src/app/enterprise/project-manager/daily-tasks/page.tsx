      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-800">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-1">
            Project Manager creates and governs the master daily work assignments. Site Manager task reviews appear in the Site Review column as soon as they are submitted.
          </p>
        </div>
          ...
          <table className="w-full text-left text-xs text-slate-300">
            <thead className="bg-slate-950/80 text-slate-400 uppercase tracking-wider font-semibold border-b border-slate-800/80">
              <tr>
                ...
                <th className="px-4 py-3.5">Site Review Status</th>
              ...
              {filteredTasks.map(task => (
                <tr key={task.id} className="hover:bg-slate-800/30 transition">
                  ...
                  <td className="px-4 py-3.5">
                    ...
                    {task.governmentRemarks && (
                      <p className="text-[10px] text-slate-500 mt-1 max-w-[180px] truncate" title={task.governmentRemarks}>
                        {task.governmentReviewedBy ? `${task.governmentReviewedBy}: ` : ''}
                        {task.governmentRemarks}
                      </p>
                    )}
                  </td>
