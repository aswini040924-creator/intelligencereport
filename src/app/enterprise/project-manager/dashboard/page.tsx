...
export default function PMDashboardPage() {
  ...
  const latestSiteReview = [...tasks]
    .filter(t => t.governmentReviewedBy && t.governmentReviewStatus !== 'PENDING_REVIEW')
    .sort((a, b) => (b.governmentReviewedAt || '').localeCompare(a.governmentReviewedAt || ''))[0];
  ...
  return (
    <div className="space-y-8">
      ...
      {latestSiteReview && (
        <div className="p-5 rounded-2xl bg-gradient-to-r from-cyan-950/30 via-slate-900/80 to-slate-900/80 border border-cyan-500/30 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="space-y-1">
            <div className="flex items-center gap-2">
              <HardHat className="w-4 h-4 text-cyan-400" />
              <span className="text-xs font-bold text-cyan-300 font-mono">
                SITE MANAGER TASK REVIEW SYNCED
              </span>
            </div>
            <h3 className="text-sm font-bold text-white">
              {latestSiteReview.activityCode}: {latestSiteReview.taskName}
            </h3>
            <p className="text-xs text-slate-400">
              {latestSiteReview.governmentReviewedBy} marked this task {latestSiteReview.governmentReviewStatus.replaceAll('_', ' ').toLowerCase()}
              {latestSiteReview.governmentRemarks ? `. Remarks: "${latestSiteReview.governmentRemarks}"` : '.'}
            </p>
          </div>

          <Link
            href="/enterprise/project-manager/daily-tasks"
            className="px-5 py-2.5 rounded-xl bg-cyan-600 hover:bg-cyan-500 text-white font-bold text-xs shrink-0 flex items-center justify-center gap-2 transition"
          >
            <span>Open Daily Tasks Board</span>
            <ArrowRight className="w-4 h-4" />
          </Link>
        </div>
      )}
