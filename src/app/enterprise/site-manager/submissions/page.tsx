...
import { Camera, Image as ImageIcon, MapPin, Clock, ArrowRight, ShieldCheck, CheckCircle2 } from 'lucide-react';

export default function SiteManagerSubmissionsPage() {
  const { submissions } = useSubmissions('PRJ-ASSAM-025');
  const { isApproved, approvedEvidence } = useEvidence({
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-slate-800">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-0.5">
            Track submitted reports, AI matching results, and Project Manager evidence validation status.
          </p>
        </div>
      ...
      <div className="space-y-4">
        {submissions.map(sub => {
          const isEvidenceApproved =
            isApproved(sub.id) ||
            isApproved(sub.taskId) ||
            approvedEvidence.some(
              e => e.submissionId === sub.id || e.taskId === sub.taskId || e.activityCode === sub.activityCode
            );
          ...
          return (
            ...
            >
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {isEvidenceApproved && (
                      <span className="px-2 py-0.5 rounded-full bg-emerald-500/15 text-emerald-300 border border-emerald-500/30 text-[10px] font-mono font-bold flex items-center gap-1">
                        ...
                        <span>Field Evidence PM-Approved</span>
                      </span>
                    )}
                  </div>
              ...
              {/* PM Approval Callout if approved */}
              {isEvidenceApproved && (
                <div className="p-3 rounded-xl bg-gradient-to-r from-emerald-950/40 to-slate-900 border border-emerald-500/30 flex items-center justify-between text-xs">
                  <div className="flex items-center gap-2">
                    <ShieldCheck className="w-4 h-4 text-emerald-400 shrink-0" />
                    <span className="text-emerald-200 font-medium">
                      Project Manager has verified and officially accepted the attached field evidence.
                    </span>
                  </div>
                  <span className="font-mono text-[10px] font-bold text-emerald-400 px-2 py-0.5 rounded bg-emerald-500/10 border border-emerald-500/20 shrink-0">
                    PM APPROVED
                  </span>
                </div>
              )}
