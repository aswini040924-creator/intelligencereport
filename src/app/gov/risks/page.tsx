...
import { Flame, ShieldAlert, AlertTriangle, ShieldCheck } from 'lucide-react';
import { cn } from '@/lib/utils';

export default function GovRisksPage() {
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-800">
        <div>
          <h1 className="text-2xl font-bold text-white tracking-tight">
            Corridor Risk Intelligence & Mitigation Registry
          </h1>
          <p className="text-xs text-slate-400 mt-1">
            Government oversight of high-impact environmental, engineering, and clearance hazards.
          </p>
        </div>
      </div>
          ...
          >
            <div>
              ...
              <div className="mt-4 p-3 rounded-xl bg-slate-950/80 border border-slate-800/80 space-y-1.5 text-xs">
                <div>
                  <span className="text-slate-500 font-semibold block text-[10px]">Impact Assessment:</span>
                  <span className="text-red-300 font-medium">{risk.impact}</span>
                </div>
                <div>
                  <span className="text-slate-500 font-semibold block text-[10px]">Contractor Mitigation Plan:</span>
            ...
            <div className="flex items-center justify-between text-[11px] text-slate-500 pt-3 border-t border-slate-800/60">
              <span>Reported: {risk.reportedBy}</span>
              <span className="font-mono text-slate-400">Status: {risk.status}</span>
            </div>
          </div>
        ))}
      </div>
