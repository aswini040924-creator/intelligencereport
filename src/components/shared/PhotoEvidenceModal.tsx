...
import { PhotoEvidence, DailyTask, SiteSubmission } from '@/types';
          ...
          <div className="flex-1 flex flex-col min-h-0 overflow-hidden">
            ...
            <div className="flex-1 grid grid-cols-1 lg:grid-cols-12 min-h-0 overflow-hidden">
              ...
              <div className="lg:col-span-5 flex flex-col min-h-0 bg-slate-900/40 overflow-y-auto p-4 sm:p-5 space-y-4">
                {/* PM Official Approval Stamp Card */}
                {(() => {
                  const approvalsMap = evidenceApi.getPhotoApprovals();
                  const photoApproval = approvalsMap[activePhoto.fileId || activePhoto.id] || (task ? approvalsMap[task.id] : undefined);
                  const isApproved = activePhoto.pmApprovalStatus === 'APPROVED' || photoApproval?.status === 'APPROVED';
                  ...
                  return (
                    <div
                      className={cn(
                        ...
                        isApproved
                          ? 'bg-gradient-to-r from-emerald-950/40 via-slate-950/80 to-slate-950/80 border-emerald-500/40 shadow-lg shadow-emerald-950/20'
                          : 'bg-slate-950/80 border-slate-800'
                      )}
                    >
                      <div className="flex items-center justify-between text-xs">
                        <span className="font-semibold flex items-center gap-1.5 text-white">
                          <ShieldCheck className={cn('w-4 h-4', isApproved ? 'text-emerald-400' : 'text-slate-400')} />
                          PM Field Evidence Validation
                        </span>
                        <span
                          className={cn(
                      ...
                      <p className="text-xs text-slate-300 leading-relaxed">
                        {isApproved
                          ? `Visual evidence validated and officially approved by ${photoApproval?.approvedBy || activePhoto.pmApprovedBy || 'Project Manager'}. Recorded for National Oversight compliance.`
                          : 'Awaiting formal Project Manager sign-off from Field Photographic Evidence console.'}
                      </p>

                      {isApproved && (
                        <div className="text-[10px] font-mono text-emerald-400/90 pt-1.5 border-t border-emerald-500/20 flex items-center justify-between">
                          <span>Audit Status: Accepted</span>
                          <span>
                            {photoApproval?.approvedAt
                              ? new Date(photoApproval.approvedAt).toLocaleDateString()
                              : 'Certified Today'}
                          </span>
                        </div>
                      )}
