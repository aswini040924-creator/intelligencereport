    ...
    case 'GOVERNMENT_APPROVED':
      return (
        <span
          ...
          title={
            isSite
              ? 'Site Manager reviewed and accepted this field report. The Project Manager board reflects the same status.'
              : 'Government Acceptance: The reported field evidence has been officially reviewed and accepted. Engineering baseline schedule remains intact.'
          }
        >
          ...
          <span>
            {isSite
              ? detailed
                ? 'Site Review Accepted'
                : 'Site Approved'
              : detailed
              ? 'Gov Monitoring Accepted (Baseline Intact)'
              : 'Gov Approved'}
          </span>
        </span>
      );
    ...
    case 'CLARIFICATION_REQUESTED':
      return (
        <span
          ...
          title={
            isSite
              ? 'Site Manager requested clarification from the Project Manager.'
              : 'Government Authority has requested formal engineering clarification before acceptance.'
          }
        >
          <HelpCircle className={size === 'sm' ? 'w-3.5 h-3.5' : 'w-4 h-4'} />
          <span>{isSite ? 'Clarification to PM' : 'Clarification Requested'}</span>
        </span>
      );
    ...
    case 'GOVERNMENT_REJECTED':
      return (
        <span
          ...
          title={
            isSite
              ? 'Site Manager rejected this field claim pending rework.'
              : 'Government rejected the reported field claim due to lack of verifiable evidence.'
          }
        >
          <XCircle className={size === 'sm' ? 'w-3.5 h-3.5' : 'w-4 h-4'} />
          <span>{isSite ? 'Site Rejected' : 'Gov Rejected'}</span>
        </span>
      );
    ...
    default:
      return (
        <span
          ...
          title={isSite ? 'Awaiting Site Manager task review' : 'Awaiting Government Compliance Officer Review'}
        >
          <Clock className={size === 'sm' ? 'w-3.5 h-3.5' : 'w-4 h-4'} />
          <span>{isSite ? 'Site Review Pending' : 'Gov Review Pending'}</span>
        </span>
      );
  }
}
