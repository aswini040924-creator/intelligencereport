...
export function useReviews() {
  ...
  // 3. Site Manager task review — persisted to the same task record the Project Manager reads
  const siteManagerReviewTask = useCallback(
    (
      taskId: string,
      action: 'APPROVE_REPORT' | 'REJECT_REPORT' | 'REQUEST_CLARIFICATION',
      remarks: string,
      siteManagerName = 'Vikramjit Singh'
    ) => {
      setIsProcessing(true);
      const tasks = storage.getLocal<DailyTask[]>(STORAGE_KEYS.TASKS, INITIAL_DAILY_TASKS);
      const audits = storage.getLocal<AuditLogEntry[]>(STORAGE_KEYS.AUDIT, INITIAL_AUDIT_LOGS);
      ...
      let nextReviewStatus: DailyTask['governmentReviewStatus'] = 'PENDING_REVIEW';
      if (action === 'APPROVE_REPORT') nextReviewStatus = 'GOVERNMENT_APPROVED';
      else if (action === 'REJECT_REPORT') nextReviewStatus = 'GOVERNMENT_REJECTED';
      else if (action === 'REQUEST_CLARIFICATION') nextReviewStatus = 'CLARIFICATION_REQUESTED';

      const updatedTasks = tasks.map(t => {
        if (t.id === taskId) {
          return {
            ...t,
            governmentReviewStatus: nextReviewStatus,
            governmentRemarks: remarks,
            governmentReviewedAt: new Date().toISOString(),
            governmentReviewedBy: siteManagerName,
            lastUpdated: new Date().toISOString(),
          };
        }
        return t;
      ...
      const newAuditLog: AuditLogEntry = {
        ...
        role: 'SITE_MANAGER',
        action: action === 'APPROVE_REPORT' ? 'SITE_MANAGER_TASK_APPROVED' : `SITE_MANAGER_${action}`,
        entity: 'DAILY_TASK',
        entityId: taskId,
        details: `Site Manager ${siteManagerName} reviewed ${task.activityCode} (${task.taskName}): ${action}. Reported: ${task.reportedProgress}%, PM Validated: ${task.validatedProgress}%. Remarks: "${remarks}"`,
        previousValue: `Review Status: ${task.governmentReviewStatus}`,
        newValue: `Review Status: ${nextReviewStatus}`,
        sha256Hash: Array.from({ length: 64 }, () => Math.floor(Math.random() * 16).toString(16)).join(''),
        status: 'VERIFIED',
      };
      storage.setLocal(STORAGE_KEYS.AUDIT, [newAuditLog, ...audits]);
      ...
      const newNotif: NotificationItem = {
        id: `NOTIF-${Date.now()}`,
        recipientRole: 'PROJECT_MANAGER',
        title: `Site Task Review: ${task.activityCode}`,
        message: `${siteManagerName} marked ${task.activityCode} as ${nextReviewStatus}. Remarks: "${remarks}".`,
        timestamp: 'Just now',
        read: false,
        type: action === 'APPROVE_REPORT' ? 'SUCCESS' : 'WARNING',
        link: '/enterprise/project-manager/daily-tasks',
